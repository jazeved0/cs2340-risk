...
case class GameContext(state: GameState, private val packetOrder: List[PacketContext]) {
  ...
  def thenSend(packet: OutPacket, recipientId: String): GameContext =
    GameContext(state, PacketContext(packet, Some(recipientId), Send) :: packetOrder)
  ...
  def thenBroadcast(packet: OutPacket, recipientId: String = ""): GameContext = recipientId match {
    case "" => GameContext(state, PacketContext(packet, None,     Broadcast) :: packetOrder)
    case id => GameContext(state, PacketContext(packet, Some(id), Broadcast) :: packetOrder)
  }
  ...
  def thenSendAll(source: GameContext): GameContext =
    GameContext(state, source.packetOrder ::: packetOrder)
  ...
  /**
    ...
    */
  @Pure
  def withState(newState: GameState): GameContext = GameContext(newState, packetOrder)
  ...
  /**
    ...
    */
  @Pure
  def map(func: GameState => GameState): GameContext = GameContext(func(state), packetOrder)
